...
import {getSingleProduct} from '../../client/store/singleProduct';
        ...
        describe('visual content', () => {
            ...
            beforeEach('Create <SingleProduct /> wrapper', () => {
                singleProductData = {
                    id: 5,
                    title: 'Straw',
                    description: 'A plastic tube that is used to drink liquid.',
                    price: '1',
                    quantity: '500'
                };
                // creates the testable React component
                singleProductWrapper = shallow(<SingleProduct fullSingleProduct={singleProductData} />);
            });

            it('includes "TITLE" line as an h3', () => {
                expect(singleProductWrapper.find('h3')).to.have.html('<h3>Straw</h3>');
            });

            it('includes "DESCRIPTION" line as p', () => {
                expect(singleProductWrapper.find('p')).to.have.html('<p>A plastic tube that is used to drink liquid.</p>');
            });

            it('includes "QUANTITY" line as h5', () => {
                expect(singleProductWrapper.find('h3')).to.have.html('<h5>Quantity in stock: 500</h5>');
            });

            it('includes "PRICE" as h5', () => {
                expect(singleProductWrapper.find('h5')).to.have.html('<h5>Price: $1</h5>');
            });
